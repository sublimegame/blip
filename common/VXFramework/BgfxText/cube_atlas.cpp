...
Atlas::Atlas(uint16_t _textureSize, uint16_t _maxRegionsCount, bool filtering)
	: m_usedLayers(0)
	, m_usedFaces(0)
	...
	, m_maxRegionCount(_maxRegionsCount)
{
	...
	m_textureHandle = bgfx::createTextureCube(_textureSize
		...
		, filtering ? BGFX_SAMPLER_NONE : BGFX_SAMPLER_POINT);
}

Atlas::Atlas(uint16_t _textureSize, const uint8_t* _textureBuffer, uint16_t _regionCount, const uint8_t* _regionBuffer, uint16_t _maxRegionsCount, bool filtering)
	: m_usedLayers(24)
	, m_usedFaces(6)
	, m_textureSize(_textureSize)
...
void Atlas::packFaceLayerUV(uint32_t _idx, uint8_t* _vertexBuffer, uint32_t _offset, uint32_t _stride, bool colored, bool filtering) const
{
	packUV(m_layers[_idx].faceRegion, _vertexBuffer, _offset, _stride, colored, filtering);
}

void Atlas::packUV(uint16_t _regionHandle, uint8_t* _vertexBuffer, uint32_t _offset, uint32_t _stride, bool colored, bool filtering) const
{
	const AtlasRegion& region = m_regions[_regionHandle];
	packUV(region, _vertexBuffer, _offset, _stride, colored, filtering);
}
...
void Atlas::packUV(const AtlasRegion& _region, uint8_t* _vertexBuffer, uint32_t _offset, uint32_t _stride, bool colored, bool filtering) const {
    ...
    int16_t ww = (int16_t)(colored + filtering * 2);
