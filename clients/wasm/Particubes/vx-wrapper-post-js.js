...
function init() {
    // Setup and mount persistent /storage directory
    FS.mkdir('/storage');
    FS.mount(IDBFS, { autoPersist: true }, '/storage');
    
    // Sync from storage to emscripten
    let populate = true;
    FS.syncfs(populate, function (err) {
        if (err != null) {
            console.error("[Cubzh][FS] init: ", "ERROR (" + err + ")")
        }
    });
...
function fs_sync_to_disk() {
    // Note: no manual sync, it is done through the 'autoPersist' FS.mount() option, this is to avoid
    // overlapping FS.syncfs() calls when we call it manually many times
    //
    // Also, there is even an argument for calling FS.syncfs() only ONCE when the app shuts down, and
    // leaving 'autoPersist' to false
    ...
    FS.syncfs(populate, function (err) {
        if (err != null) {
            console.error("[Cubzh][FS] fs_sync_to_disk: ", "ERROR (" + err + ")")
        }
    }); */
}
